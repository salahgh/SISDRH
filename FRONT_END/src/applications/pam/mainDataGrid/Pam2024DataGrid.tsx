...
import {
  downloadPhoto,
  highlightSearchTokenWithSpan,
} from "../../common/utilities/tools.ts";
...
export const Pam2024DataGrid = () => {
  ...
  const {
    ...
    selectedPersonel,
    setSelectedPersonel,
    avatarEditorDialogueOpen,
    setAvatarEditorDialogueOpen,
    rowCount,
    setRowCount,
    noteDialogueOpen,
    setNoteDialogueOpen,
    setPersonelCardOpen,
    personelCardOpen,
    photoEditorDialogueOpen,
    setPhotoEditorDialogueOpen,
  } = usePam2024DataGridState();

  const { pam2024, refetch, loading, error, handleSubmit } =
    usePam2024DataGridDataService(page);
  ...
  const handleAvatarClicked = (
    ...
    if (altKeyPressed && hovered) {
      downloadPhoto(row?.matricule).then((r) =>
      ...
      renderCell: ({ row }: { row: PamOff2024 }) => (
        <Stack direction={"row"} spacing={1} alignItems={"center"}>
          <PamDataGridAvatarPhoto
            onClick={(e, altKeyPressed, hovered) =>
              handleAvatarClicked(e, altKeyPressed, hovered, row)
            }
            row={row}
            size={48}
            handleDoubleClick={() => {
          ...
          {row?.matricule !== directeur && (
            <RetretableChip matricule={row?.matricule}></RetretableChip>
          )}
        </Stack>
      ),
    },
          ...
          <ListItemText
            ...
            secondary={
              <Matricule
                ...
                highlight={filteringFields.searchToken}
              />
            }
          ></ListItemText>
        </ListItem>
      ...
      renderCell: ({ row }: { row: PamOff2024 }) => (
        ...
        >
          <Typography fontWeight={"bold"}>
            {row?.structureSn?.runite?.abreviationAr +
              "/" +
              row?.structureSn?.runite?.regionMilitaire?.libAbrRegionAr}
          </Typography>
          <ArrowBack></ArrowBack>
          <Typography>{row?.fonction}</Typography>
        </Stack>
      ),
    },
    // {
    ...
    // },
    {
      ...
      renderCell: ({ row }: { row: PamOff2024 }) => (
        <ListItem>
          ...
          <ListItemText
            primary={
              <Typography fontWeight="bold">{row?.dipmil?.abrFr}</Typography>
            }
          ></ListItemText>
        </ListItem>
      ),
    },
    {
      ...
      renderCell: ({ row }: { row: PamOff2024 }) => (
        <ListItem>
          ...
          <ListItemText
            primary={<Typography>{row?.dipcivil?.abrFr}</Typography>}
          ></ListItemText>
        </ListItem>
      ),
    },
  ...
  return (
    <Stack sx={{ width: "100%", height: "100%" }} padding={1} spacing={1}>
