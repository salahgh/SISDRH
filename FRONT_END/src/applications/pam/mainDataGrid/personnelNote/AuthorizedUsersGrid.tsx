...
export const AuthorizedUsersGrid = ({selectedNote , handleAddAuthorizedUser , handleDeleteAuthrizedUser}) => {
    ...
    if (selectedNote) {
        ...
        if (sortedAuthorizedUsers) {
            sortedAuthorizedUsers = sortedAuthorizedUsers.sort(
                (a, b) => a?.personnel?.pamOff2024?.trieAnciennete > b?.personnel?.pamOff2024?.trieAnciennete
            );
        }
    }
        ...
        <Grid2 container={true} spacing={1}>
            ...
            </Grid2>
            {
                sortedAuthorizedUsers?.map((user, index) => {
                    return (
                        <Grid2>
                           <AuthorizedUserCard user={user} index={index} handleDelete={handleDelete}/>
                        </Grid2>
                    )
                })
            }
