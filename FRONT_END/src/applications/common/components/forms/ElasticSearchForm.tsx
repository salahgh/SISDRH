...
import { Stack, ToggleButton, Typography, Button, Paper, Tooltip } from "@mui/material";
import { TextField, ToggleButtonGroup } from "formik-mui";

import { useQuery } from "@apollo/client";
import {
  Deselect,
  Error, ExpandLess, ExpandMore, RestartAltRounded,
  SearchRounded
...
import {
  ConfidentialiteCountDocument, TypeTextReglementaireCountDocument
} from "../../../../_generated_gql_/graphql.ts";
import { DatePicker } from "formik-mui-x-date-pickers";
import {
  ConfidentialiteChip
} from "../../../textReglementaire/pages/FoldersTexteReglementaire/pdfFiles/ConfidentialiteChip.tsx";
...
export default function ElasticSearchForm({ handleSubmit, isLoading }) {
  ...
  const {
    ...
    loading: loadingConfidentialites
  } = useQuery(ConfidentialiteCountDocument);

  useEffect(() => {
    console.log("initial use effect for all confidentialites and alltypetextereglementaires");
    if (allConfidentialites && allTypeTexteReglementaires) {
      console.log(allConfidentialites, allTypeTexteReglementaires);
      dispatch(setelasticSearchInput({
        ...elasticSearchInput,
        idsTypeTextReglementaire: allTypeTexteReglementaires.typeTextReglementaireCount.map((item) => item?.libTypeTexteFr),
        isConfidentialite: allConfidentialites.confidentialiteCount.map((item) => item?.libConfidentialiteFr)
      }));
      setInitialValues({
        ...elasticSearchInput,
        idsTypeTextReglementaire: allTypeTexteReglementaires.typeTextReglementaireCount.map((item) => item?.id),
        isConfidentialite: allConfidentialites.confidentialiteCount.map((item) => item?.id)
      });
    }
    ...
    <Formik initialValues={initialValues} onSubmit={handleSubmit}>
      {({
        ...
        return (
          <Form>
            <Paper
              className={"shadow-lg drop-shadow-lg p-2 space-y-2"}
            >
              <Stack direction={"row"} spacing={1}
              >
                <Stack>
                  <Button onClick={() => dispatch(open ? closeSearchPanel() : openSearchPanel())}>
                    {
                      open ? <ExpandLess></ExpandLess> : <Stack direction={"row"} spacing={2}>
                ...
                >
                  ...
                  >
                    <Button
                      disabled={arraysContainSameElements(values.isConfidentialite, [])}
                    ...
                    >
                      ...
                      {
                        loadingConfidentialites && <LoadingConfidentialiteButtons size={4} />
                      }
                    </Field>
                  </Stack>
                  ...
                  >
                    ...
                    <Field
                      ...
                      type="checkbox"
                    >
                      {allTypeTexteReglementaires?.typeTextReglementaireCount?.map((option) => (
                        <Tooltip title={option?.libTypeTexteFr + " au nombre de " + option?.count_}>
                          <ToggleButton
                            key={option.id}
                      ...
                      {
                        loadingAllTypeTextReglementaires && <LoadingConfidentialiteButtons width={60} size={6} />
                      }
                    </Field>
                  </Stack>
