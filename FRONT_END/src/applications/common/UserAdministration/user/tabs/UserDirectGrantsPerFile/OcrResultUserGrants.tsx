    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <OcrResultUserGrantsAvatarGroup
          ...
          borderColor={"#2ea100"}
          size={60}
          userIds={row?.ocrResultUserGrants
            ?.filter((grant) => grant?.id?.type === "GRANT")
            .map((grant) => grant?.user?.personnel?.matricule)}
          onClick={() => {
      ...
      renderCell: (rowParams) => (
        <Stack direction={"row"}>
          <OcrResultUserGrantsAvatarGroup
            ...
            borderColor={"#880000"}
            size={60}
            userIds={rowParams?.row?.ocrResultUserGrants
              ?.filter((grant) => grant?.id?.type === "REVOKE")
              .map((grant) => grant?.user?.personnel?.matricule)}
            onClick={() => {
    ...
    >
      ...
      <Paper sx={{ flex: 1, height: "100%" }}>
        <DataGrid
          ...
          rowHeight={80}
          rows={
            data?.allOcrResultUsersGrantsPaged?.content
              ? data?.allOcrResultUsersGrantsPaged?.content
              : []
          }
          columns={columns}
          ...
          onPageSizeChange={(newPageSize) => {
            setPaginationInput((pageable) => ({
              ...pageable,
              pageSize: newPageSize,
            }));
          }}
          onPageChange={(newPage) =>
            setPaginationInput((old) => ({ ...old, pageNumber: newPage }))
          }
