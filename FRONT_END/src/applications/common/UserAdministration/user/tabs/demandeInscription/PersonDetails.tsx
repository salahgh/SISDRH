...
import { GetPhotoByMatriculeAndSizeDocument } from "../../../../../../_generated_gql_/graphql.ts";
...
const PersonDetails = ({
  ...
  const { data, loading, error } = useQuery(
    ...
    { variables: { matricule: person?.matricule, size: 500 } },
  );
      ...
      {person && !error && (
        <>
          <CardActionArea
            ...
            sx={{
              bgcolor:
                selectedPseronnel?.matricule == person?.matricule
                  ? "rgba(141,141,141,0.36)"
                  : null,
            }}
          >
            {!loading ? (
              <CardMedia
                ...
                height={width_ * 1.2}
                width={width_}
                image={TOOLS.getSrcFromBase64String(
                  data?.getPhotoByMatriculeAndSize?.data,
                )}
                alt={person?.matricule}
              />
            ) : (
              <Box sx={{ height: 1.2 * width_ }}>
                <Skeleton variant={"circular"} sx={{ height: 1.2 * width_ }} />
              </Box>
            )}
            <CardContent>
