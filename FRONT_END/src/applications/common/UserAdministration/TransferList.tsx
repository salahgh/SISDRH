...
export default function TransferList({ role, setTransferListOpen }) {
  ...
  const [
    addRole,
    { data, error: addPrivilegeToRoleError, loading: addingPrivilgeToRole },
  ] = useMutation(AddPrivilegeToRoleDocument);
  const [
    deleteRole,
    { error: deletePrivilgeFromRoleError, loading: deleteingPrivilgeFromRole },
  ] = useMutation(DeletePrivilegeFromRoleDocument);

  const [left, setLeft] = useState<PrivilegeDto[] | undefined>(
    not(
      privileges?._embedded?.privilegeDtoList?.filter((privilege) =>
        privilege?.name?.includes(searchPrivilegename),
      ),
      role?.privileges,
    ),
  );
  const [right, setRight] = useState<PrivilegeDto[] | undefined>(
    role?.privileges,
  );
  ...
  const customList = (items: PrivilegeDto[], isLeft: boolean) => {
    const filteredLeft = isLeft
      ? items.filter((item) => item.name?.includes(searchPrivilegename))
      : items;
    return (
            ...
            <Stack direction={"row"}>
              <TextField
                ...
                InputProps={{
                  ...
                  endAdornment: (
                    <Search
                      sx={{ color: blue["500"], padding: 1, fontSize: 40 }}
                    />
                  ),
                }}
                size={"small"}
