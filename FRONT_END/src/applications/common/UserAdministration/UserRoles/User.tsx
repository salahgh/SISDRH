...
import {
    PrivilegesEnum,
    RolesEnum, UserAddPrivilegeDocument, UserAddPrivilegeRestrictionDocument,
    UserAddRoleDocument, UserDeletePrivilegeDocument, UserDeletePrivilegeRestrictionDocument,
    UserDocument
...
const User = ({matricule}: { matricule?: string | null }) => {
    ...
    const [addPrivilegeRestrictionOpen, setAddPrivilegeRestrictionOpen] = useState<boolean>(false)
    const [selectedRoleId, setSelectedRoleId] = useState<string | null>(null)
    const location = useLocation()
    const user_location = location.state && location.state.user && location.state.user
    ...
    const handleDeletePrivilege = (privilege) => {
        userDeletePrivilege({
            variables: {
                userName: matricule, privilegeName: privilege?.name
            }
    ...
    const [userAddPrivilegeRestriction] = useMutation(UserAddPrivilegeRestrictionDocument, {
        refetchQueries: [{
            query: UserDocument,
            variables: {
                matricule: matricule
    ...
    const [userDeletePrivilegeRestriction] = useMutation(UserDeletePrivilegeRestrictionDocument, {
        refetchQueries: [{
            query: UserDocument,
            variables: {
                matricule: matricule
            ...
            >
                ...
                >
                    <AvatarPhoto
                        ...
                        borderRadius={10}
                    />
                    <GetStackEditable username={user?.user?.personnel?.matricule}></GetStackEditable>
                </Stack>
                <Stack sx={{width : '25%'}} spacing={2}>
                    ...
                    >ajouter</Button>
                    {
                        user?.user?.roles?.length === 0 ? <div>
                            <img src={ASSETS.emptyInbox} alt={'empty'} style={{width : '70px'}} />
                        </div> : <>
                            <ListOFRoles
                ...
                <Stack sx={{width : '25%'}}>
                    {
                        user?.user?.privileges?.length === 0 ? <div>
                            <img src={ASSETS.emptyInbox} alt={'empty'} style={{width : '70px'}} />
                        </div> : <>
                            {
                                selectedRoleId && <div>
                                    <RoleListItem
                                        role={user?.user?.roles?.filter((role) => role?.id === selectedRoleId)[0]}
                                        displayId={true}
                                    />
                                    <ListOFPrevileges
                                        privileges={user?.user?.roles?.filter((role) => role?.id === selectedRoleId)[0]?.privileges}
                                    />
                                </div>
                            }
                        </>
                ...
                <Stack sx={{width : '25%'}}>
                    ...
                    </Button>
                    {
                        user?.user?.privileges?.length === 0 ? <div>
                            <img src={ASSETS.emptyInbox} alt={'empty'} style={{width : '70px'}} />
                        </div> : <ListOFPrevileges handleDelete={handleDeletePrivilege} privileges={user?.user?.privileges}/>
                    }
                </Stack>
                ...
                <Stack sx={{width : '25%'}}>
                    ...
                    {
                        user?.user?.privileges_restriction?.length === 0 ? <div>
                            <img src={ASSETS.emptyInbox} alt={'empty'} style={{width : '70px'}} />
                        </div> : <ListOFPrevileges handleDelete={handleDeletePrivilegeRestriction}
