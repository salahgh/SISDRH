...
export default function PdfViewerToggleButton({ viewers }) {
  ...
  return (
    ...
    >
      {
        viewers?.some((item) => item === "PDF") && <ToggleButton value="PDF" aria-label="left aligned">
          <PictureAsPdf
      ...
      {
        viewers?.some((item) => item === "IMAGE") &&  <ToggleButton value="IMAGE" aria-label="centered">
          <Wallpaper
      ...
      {
        viewers?.some((item) => item === "TEXT") &&  <ToggleButton value="TEXT" aria-label="centered">
          <Description
