import {OcrResultImagePreparedDocument, OcrResultImagePreparedQuery} from "../../../../../_generated_gql_/graphql";
...
export function PagePreview(props: { fileId: string, pageIndex: number, width: string, shouldNavigate: boolean }) {
   ...
   }: { data: OcrResultImagePreparedQuery } =
   ...
   function handleShowPdf() {
      ...
      dispatch(setselectedSinglePdfViewerFileId(fileId))
      dispatch(setselectedSinglePdfViewerPageIndex(pageIndex + 1))
      ...
      <Stack sx={{
         ...
         }
         {
            imageBase64 && !imageLoading && <>
               <Card sx={{
                  bgcolor: selectedPageIndex - 1 === pageIndex ? '#707070' : '#ffffff'
               }
               } >
                  <CardActionArea sx={{padding : 0.3}} onClick={() => handleShowPdf()}>
                     <img src={"data:image/png;base64," + imageBase64.ocrResultImagePrepared}
                          alt="Element"
                          style={{
                             width: width,
                             height: width * aspectRation,
